...
const removeDirectories = (id, directories, notices) => {
  ...
  const updatedDirectories = _.reject(
    directories,
    directory => _.indexOf(deletedDirIds, directory.id) !== -1
  );
  const updatedNotices = _.reject(
    notices,
    notice => _.indexOf(deletedDirIds, notice.directoryId) !== -1
  );
...
router
  ...
  .get("/:id", function(req, res) {
    ...
    var singleDirectory = _.find(store.directories, function(dir) {
      return dir.id === directoryId;
    });

    if (singleDirectory) {
      res.send(singleDirectory);
    } else {
      res.status(500).send("no such folder");
    }
  })
  ...
  .delete("/:id", function(req, res) {
    ...
    try {
      const { updatedDirectories, updatedNotices } = removeDirectories(
        directoryId,
        store.directories,
        store.notices
      );
